...
| Script                        | Description                                                                                                                                                             |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `run_clone_measurement.sh`    | Measurement entrypoint: runs every benchmark dataset for one engine (`--engine vep-rs\|fastvep\|perl`) against a populated data tree, one discarded warmup plus one timed run, emitting `wall_times.csv` and per-dataset concordance reports; the wall times and F1 values under `manuscript/data/` are its outputs. |
